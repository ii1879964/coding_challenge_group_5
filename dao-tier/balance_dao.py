...
class BalanceDAO(object):
    ...
    def recount_profit_loss(self, new_deal):
        if not new_deal['instrument_name'] in BalanceDAO.quantity_of_shares:
            BalanceDAO.quantity_of_shares[new_deal['instrument_name']] = 0
        if not new_deal['instrument_name'] in BalanceDAO.balance_per_instrument_name:
            BalanceDAO.balance_per_instrument_name[new_deal['instrument_name']] = 0

        if (new_deal['type'] == 'B'):
            BalanceDAO.balance_per_instrument_name[new_deal['instrument_name']] -= new_deal['quantity'] * new_deal['price']
            BalanceDAO.quantity_of_shares[new_deal['instrument_name']] += new_deal['quantity']
